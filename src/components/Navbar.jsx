...
function Navbar() {
  ...
  // EFEK OTOMATIS: Tutup Menu Mobile & Ubah Judul Tab Browser (Dynamic Page Title)
  useEffect(() => {
    setIsMenuOpen(false);
    setMobileMenuOpen("");

    const brand = identitas?.namaBesar || "Enka Imron Mandiri";
    const path = location.pathname;

    if (path === "/") {
      document.title = `${brand} — ${identitas?.tagline || "Travel Umroh, Domestik & Internasional"}`;
    } else if (path === "/umroh") {
      document.title = `Paket Ibadah Umroh | ${brand}`;
    } else if (path === "/umroh/paket") {
      document.title = `Katalog Semua Paket Umroh | ${brand}`;
    } else if (path === "/domestik") {
      document.title = `Paket Wisata Domestik | ${brand}`;
    } else if (path === "/domestik/destinasi") {
      document.title = `Eksplorasi Destinasi Nusantara | ${brand}`;
    } else if (path === "/domestik/paket") {
      document.title = `Katalog Semua Paket Domestik | ${brand}`;
    } else if (path.startsWith("/domestik/daerah/")) {
      const daerah = decodeURIComponent(path.replace("/domestik/daerah/", ""));
      document.title = `Trip Wisata ${daerah} | ${brand}`;
    } else if (path === "/internasional") {
      document.title = `Paket Tour Internasional | ${brand}`;
    } else if (path === "/internasional/destinasi") {
      document.title = `Katalog Kawasan & Negara | ${brand}`;
    } else if (path === "/internasional/paket") {
      document.title = `Katalog Paket Tour Luar Negeri | ${brand}`;
    } else if (path.startsWith("/internasional/destinasi/")) {
      const kawasan = decodeURIComponent(path.replace("/internasional/destinasi/", ""));
      document.title = `Tour ${kawasan} | ${brand}`;
    } else if (path === "/berita") {
      document.title = `Berita & Artikel Perjalanan | ${brand}`;
    } else if (path === "/faq") {
      document.title = `Tanya Jawab (FAQ) | ${brand}`;
    } else if (path === "/syarat") {
      document.title = `Syarat & Ketentuan | ${brand}`;
    } else if (path === "/tentang") {
      document.title = `Tentang Kami | ${brand}`;
    }
  }, [location.pathname, identitas]);
      ...
      <nav className="fixed w-full top-0 z-[100] shadow-sm flex flex-col">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-20">
              ...
              <div className="hidden md:flex items-center space-x-8">
                ...
                <a href={activeWaLink} target="_blank" rel="noopener noreferrer" className="bg-[#f59e0b] hover:bg-yellow-500 text-black px-5 py-2.5 rounded-xl font-bold text-sm transition shadow-lg shadow-yellow-500/20 flex items-center gap-2"><MessageCircle size={18} /> Hubungi Kami</a>
              </div>
        ...
        <div className={`md:hidden absolute w-full bg-white border-b border-gray-100 shadow-2xl transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-[80vh] opacity-100 overflow-y-auto' : 'max-h-0 opacity-0 overflow-hidden'}`} style={{ top: "100%" }}>
          <div className="px-5 pt-3 pb-8 space-y-2 flex flex-col">
            ...
            <div className="pt-5 mt-3 border-t border-gray-100"><a href={activeWaLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 text-center bg-[#f59e0b] hover:bg-yellow-500 text-black px-5 py-3.5 rounded-xl font-bold text-[15px] shadow-lg shadow-yellow-500/20"><MessageCircle size={20} /> Hubungi via WhatsApp</a></div>
          </div>
        </div>
      </nav>
