...
import { 
  ...
  FileText, MessageCircle, Info, ShieldCheck, Clock 
} from "lucide-react";
...
function Navbar() {
  ...
  useEffect(() => {
    const fetchIdentitas = async () => {
      try {
        ...
        if (snap.exists()) {
          setIdentitas(snap.data());
        } else {
          setIdentitas({
            namaBesar: "ENKA IMRON MANDIRI", tagline: "Travel Domestik • Internasional • Umroh",
            taglineSize: "text-[8px] md:text-[9px]", lisensi: "Berizin Resmi Kemenag RI No. 1234 Tahun 2024",
            topBarKanan: "Layanan Pelanggan: 08.00 - 17.00 WIB", logoNavbar: ""
          });
        }
      } catch (error) { console.error("Gagal mengambil identitas:", error); }
    };
    fetchIdentitas();
  }, []);
  ...
  return (
    <nav className="fixed w-full top-0 z-[100] shadow-sm flex flex-col">
      ...
      {/* ================= TOP BAR (PITA BIRU ATAS DILURUSKAN) ================= */}
      {identitas && (
        <div className="bg-[#1e3a8a] text-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 text-[10px] md:text-xs flex justify-between items-center">
            <div className="flex items-center gap-1.5 max-w-[65%] truncate">
              <ShieldCheck size={14} className="text-[#f59e0b] shrink-0"/>
