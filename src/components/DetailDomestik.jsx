...
function DetailDomestik() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (idSnap.exists()) {
          const d = idSnap.data();
          setWaNumber(formatWaNumber(d.noWaDomestik) || formatWaNumber(d.noWa) || "6281234567890");
        }
      } catch (error) {
  ...
  const pesanPaket = `Halo Admin Divisi Domestik Enka Imron Mandiri, saya tertarik dengan paket wisata domestik:\n\n*Nama Paket:* ${paket.title}\n*Destinasi:* ${paket.daerah || "-"}\n*Kategori:* ${paket.tipeTrip || "Open Trip"} (${paket.duration || "-"})\n\nMohon informasi lebih lanjut mengenai jadwal dan ketersediaannya. Terima kasih.`;
  const linkWaPaket = `https://wa.me/${waNumber}?text=${encodeURIComponent(pesanPaket)}`;

  const pesanCustom = `Halo Admin Divisi Domestik Enka Imron Mandiri, saya sedang melihat paket *${paket.title}* dan ingin berkonsultasi untuk menyesuaikan rute/isi paket ini.`;
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {paket.hargaPrivate.map((hp, idx) => (
                    <div key={idx} className="bg-slate-50 border border-blue-100 p-4 rounded-2xl flex justify-between items-center group hover:bg-blue-50 transition-colors">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 md:p-8 sticky top-28">
             ...
             <SharePackage 
               ...
               kategori={`Trip Domestik (${paket.tipeTrip || "Open Trip"})`} 
               infoTambahan={`Destinasi: ${paket.daerah}`} 
             />
           </div>
            </div>
          </div>
        ...
        <div className="mt-16 md:mt-24">
          <div className={`relative rounded-3xl overflow-hidden shadow-xl min-h-[200px] md:min-h-[250px] flex items-center ${config?.ctaBgColor || 'bg-[#1e3a8a]'}`}>
            <div className="absolute inset-0 w-full h-full">
              <img src={config?.ctaBg || "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?q=80&w=2000"} className="w-full h-full object-cover object-center" />
            </div>
            ...
            <div className="relative z-10 w-full md:w-2/3 p-8 md:p-12 text-left">
              <h2 className="text-2xl md:text-4xl font-bold text-white mb-3 md:mb-4 leading-snug">
                {config?.ctaTitle || "Ingin Menyesuaikan Isi Paket Ini?"}
              </h2>
              <p className="text-blue-100 mb-6 text-sm md:text-base max-w-xl leading-relaxed">
                {config?.ctaDesc || "Atau ingin membuat rute perjalanan impian Anda sendiri? Konsultasikan dengan tim kami untuk mewujudkan liburan yang tak terlupakan."}
              </p>
              <a href={linkWaCustom} target="_blank" rel="noopener noreferrer" className="inline-block bg-[#f59e0b] hover:bg-yellow-600 text-white font-bold py-3 px-8 rounded-xl shadow-lg transition-colors text-sm md:text-base">
                {config?.ctaBtnText || "Konsultasi via WhatsApp"}
              </a>
            </div>
          </div>
        </div>
